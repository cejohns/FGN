  ...
  return (
    <header className="bg-fs-dark border-b border-fs-panel sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        {/* RIGHT: main nav + mobile menu button */}
      ...
      >
        <nav className="flex flex-col gap-3 text-sm">
          ...
          {/* Join button still available in mobile menu */}
          <button
            onClick={() => {
              onNavigate('releases');
              document.getElementById('mobile-nav')?.classList.add('hidden');
            }}
            className="px-4 py-2 mt-1 rounded-full bg-fs-blue text-fs-dark font-medium hover:bg-fs-blueStrong transition-colors text-center"
          >
            Join the Network
          </button>
        </nav>
