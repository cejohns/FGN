...
export default function NewsPage({ selectedArticleId, onBack }: NewsPageProps) {
  ...
  const fetchArticleById = async (id: string) => {
    ...
    try {
      ...
      const nextViewCount = ((article as any).view_count ?? 0) + 1;
  ...
  const coverUrl = (url?: string | null) =>
    url && url.trim().length > 0 ? url : 'https://placehold.co/1200x675?text=No+Image';

  const authorMeta = useMemo(() => {
    const name = (selectedArticle?.author ?? '').trim() || 'Staff';
  ...
  if (selectedArticle) {
    return (
      <div className="max-w-4xl mx-auto">
        <button
          onClick={onBack}
          className="flex items-center space-x-2 text-cyan-600 hover:text-cyan-700 font-medium mb-6"
        >
        ...
        <article className="bg-slate-950 rounded-xl shadow-lg overflow-hidden">
          <div className="aspect-video overflow-hidden">
            <ImageWithFallback
              src={coverUrl(selectedArticle.featured_image)}
          ...
          <div className="p-8">
            <div className="flex items-center space-x-4 mb-4 text-sm text-gray-500">
              ...
              <span className="flex items-center">
                ...
                {(selectedArticle as any).view_count ?? 0}
              </span>
            </div>
            ...
            <div className="flex items-center space-x-3 mb-6 pb-6 border-b border-gray-700">
              ...
              <div>
                <p className="font-medium text-white">{authorMeta.name}</p>
            ...
            <div className="prose prose-lg max-w-none text-gray-300">
              {selectedArticle.excerpt ? (
                <p className="text-xl mb-6 font-medium">{selectedArticle.excerpt}</p>
              ) : null}
  ...
  const filteredArticles =
    selectedCategory === 'All'
      ? articles
      : articles.filter((a) => (a.category ?? '').trim() === selectedCategory);
  ...
  return (
    <div>
      ...
      {filteredArticles.length === 0 ? (
        <div className="text-center py-16 bg-slate-950 rounded-xl">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredArticles.map((article) => (
            <button
              ...
              className="group bg-slate-950 rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all text-left"
            >
              <div className="aspect-video overflow-hidden">
                <ImageWithFallback
                  src={coverUrl(article.featured_image)}
